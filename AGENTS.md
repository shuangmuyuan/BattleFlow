...
5. When working from an additional git worktree, check whether the canonical repository already has a local `.env` and a running local service before creating or initializing anything:
   ...
   - for a one-off worktree run, prefer `set -a && source /Users/lichunhe/Documents/Playground/BattleFlow/.env && set +a` before `BATTLEFLOW_WORKSPACE_PATH="$(pwd)" CLAUDE_WORKSPACE_DIR="$(pwd)" BATTLEFLOW_CLAUDE_TOOLS=Read,Grep,Glob,WebSearch,WebFetch,Write,Edit DEPLOY_RUN_PORT=5101 pnpm dev`; `scripts/dev.sh` lets the Agent SDK use its bundled runtime unless a custom `CLAUDE_COMMAND` is explicitly configured;
