...
4. Keep Skill self-check default-on for every completion gate. Promote a candidate from self-check alone only when independent Agent validation is disabled; when it is enabled, require both phases to pass.
...
10. Protect workflow validation routes with organization context and workflow resource permissions before reading or mutating validation attempts.
11. Update related docs and tests when behavior, commands, states, or route contracts change.
