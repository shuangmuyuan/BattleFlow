...
| BattleFlow Agent Runtime | `.agents/skills/battleflow-agent-runtime/SKILL.md` | Procedure for Claude Agent SDK runtime, detached workflow chat runs, run event replay, node workspaces, shared artifacts, guarded node-local writes, prompt assembly, Claude auth, and agent tool permissions. |
