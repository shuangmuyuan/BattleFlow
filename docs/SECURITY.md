...
- `Read`, `Grep`, and `Glob` are allowed only for the current node cwd and explicit readable directories such as promoted workflow artifacts. The adapter denies parent traversal, absolute paths outside approved roots, symlink escapes, node metadata, sibling nodes, and repo paths before tool execution;
