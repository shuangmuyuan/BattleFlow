...
- failed `tool_result` messages remain visible as failed tool events but do not bypass policy or terminate the query by themselves. The SDK may recover with another allowed action; BattleFlow marks the turn failed only for a top-level SDK error result, a thrown stream error, explicit cancellation, or abort;
