...
- It uses `tools` to restrict the available built-in tool set to the explicit `BATTLEFLOW_CLAUDE_TOOLS` allowlist, and mirrors that list in `allowedTools` only for auto-approval. When a node chat turn has a HITL handler, `Write` and `Edit` remain available through `tools` but are removed from `allowedTools` so SDK `canUseTool` can request approval after server path validation passes.
...
- Node chat turns may use configured `Write` and `Edit` tools only when the target path resolves inside the current node cwd. The SDK adapter enforces this with both `canUseTool` and `PreToolUse`; `.claude/`, node metadata, shared `artifacts/`, sibling nodes, and repo paths are denied before any user approval prompt is shown. `MultiEdit` and `Bash` remain unsupported. Non-node calls that do not pass a Skill continue to disallow `Skill`, `Write`, and `Edit`.
