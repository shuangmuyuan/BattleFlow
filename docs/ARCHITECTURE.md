...
- Node chat turns may use configured `Write` and `Edit` tools only when the target path resolves inside the current node cwd. The SDK adapter enforces this with both `canUseTool` and `PreToolUse`; `.claude/`, node metadata, shared `artifacts/`, sibling nodes, and repo paths are denied. `MultiEdit` and `Bash` remain unsupported. Non-node calls that do not pass a Skill continue to disallow `Skill`, `Write`, and `Edit`.
